...
export function RoomWeekGrid({ model, today }: { model: RoomWeekModel; today: string }) {
  ...
  const cols = `3.5rem repeat(${days.length}, minmax(5.25rem, 1fr))`;

  return (
    <div className="overflow-x-auto rounded-lg border border-hairline bg-surface-0">
      <div className="grid min-w-[760px]" style={{ gridTemplateColumns: cols }}>
        ...
        {days.map((d) => {
          ...
          return (
            ...
            >
              ...
              <div className="numeral text-sm tabular-nums opacity-80">
        ...
        {blocks.map((start, i) => (
          <Fragment key={start}>
            <div className="numeral sticky left-0 z-10 flex items-start justify-end border-t border-hairline bg-surface-0 px-1.5 py-1 text-[0.72rem] tabular-nums text-text-muted">
              {start}
            </div>
            {days.map((d) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[2.25rem] border-t border-l border-hairline px-1.5 py-1.5 ${STATE_CLS[cell.state]}`}
                >
                  <span className="flex items-center gap-0.5 leading-tight">
                    {Icon && <Icon size={11} weight="bold" aria-hidden className="shrink-0 opacity-70" />}
                    {cell.recurring && (
                      <ArrowsClockwise size={11} weight="bold" aria-hidden className="shrink-0 text-gold" />
                    )}
                    {cell.label && (
                      <span className="truncate text-[0.72rem] font-medium">{cell.label}</span>
                    )}
                  </span>
                </div>
              );
